...
public class PlateauAwale implements PlateauJeu{
    /* 
       Note : Pour l'heuristique, on pourrait stocker "l'avancement" de la partie, pour lancer des heuristiques différentes suivant si on est au début, au milieu, à la fin de la partie 
*/
    ...
    */
    ...
    private static final int idJ1 = 0,
	idJ2 = 1;
	...
	int nb_j = coup.getNumJoueur(),
	    case_ = coup.getNumCase(),	    
	    nb_graines = plateau[ nb_j ][ case_ ],
	    cpt_case = case_, 
	    cpt_joueur = nb_j ;
	...
	// 2 : Enlèvement des graines si on est dans le camp qui vient pas de jouer + qu'il y en a deux ou trois 

	PlateauAwale etat_plateau = (PlateauAwale) this.copy();
	while( cpt_joueur != nb_j
	       && ( plateau[ cpt_joueur ][ cpt_case ] == 2
		    || plateau[ cpt_joueur ][ cpt_case ] == 3)){
	...
	return ( (coupsPossibles(j1).size() == 0 && coupsPossibles(j2).size() == 0)
		 || nbGrainesJ1 > 24
		 || nbGrainesJ2 > 24
		 || this.nb_graines_restantes() <= 6
		 ) ;
    }
	...
	if( ! hasNothing(autre_joueur) ) return true;
	
	if( hasNothing(autre_joueur) &&
	    (ind_joueur == 0 && (id_case + nb_graines_sur_case > NB_CASES_LARGEUR))
	    ||
	...
	for(int i=0; i<6; i++)
	    if( plateau[idJoueur][i] != 0) return false;
	return true;
    }
